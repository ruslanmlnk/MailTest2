...
interface IGetAQuoteNavbarPopup {
	isOpened: boolean;
...
const GetAQuoteNavbarPopup: FC<IGetAQuoteNavbarPopup> = ({
	isOpened,
	setIsOpened,
	text,
	display,
						...
						onSubmit={(e) => {
							if (validatePhoneNumber(formsStore.GetAQuoteData.PhoneNumber)) {
								formsStore.sendQuote(
									"http://localhost:5177/send/movingRequest",
					...
					>
						...
						>
							{formsStore.loadingRequest ? (
								...
								<>
									Submit <Image src={ButtonArrow} alt="arrow-icon" />
								</>
							)}
						</button>
					</form>
...
export default observer(GetAQuoteNavbarPopup);
